...
1️⃣ Check if Streamlit is installed
...
If it shows package info → it is installed, skip to Step 3

If it says Package not found → continue to Step 2
...
3️⃣ Run Streamlit using Python (most reliable fix)

If streamlit is still not recognized, run it this way:

python -m streamlit run main.py


✅ This works even when PATH is not set correctly.

4️⃣ (Optional but Recommended) Fix PATH permanently

If you want streamlit run main.py to work directly:

Find where Python scripts are installed, usually:
...
5️⃣ If you are using a virtual environment
...
1️⃣ Install tf-keras (backward-compatible Keras)
...
That’s exactly what the error message is asking you to do.

2️⃣ Restart terminal (important)

Close CMD / PowerShell completely and reopen it.
...
🧪 If it STILL fails (very common with Python 3.12)

Then do the stable academic-project setup 👇
This is what most ML libraries currently work best with.

🟢 STABLE SETUP (Highly Recommended for your project)
...
pip install tf-keras
pip install transformers sentence-transformers keybert streamlit


⚠️ TensorFlow 2.15 is the last version before Keras 3 takeover.
